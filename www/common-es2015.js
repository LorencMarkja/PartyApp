...
let AuthService = class AuthService {
    constructor(http) {
        this.http = http;
        this.url = 'http://localhost:8888/auth_app/api';
    }
    register(user) {
        return this.http.post(`${this.url}/register.php`, user);
    }
    login(credentials) {
        return this.http.post(`${this.url}/login.php`, credentials).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["map"])(response => response.token));
    }
};
